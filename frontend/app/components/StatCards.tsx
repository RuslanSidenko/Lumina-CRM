...
export default function StatCards({ leads = [], properties = [] }: StatCardsProps) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div className="glass-panel p-6 flex flex-col gap-2 relative overflow-hidden group">
        ...
        <span className="text-sm font-medium text-slate-400">Total Active Leads</span>
        <span className="text-4xl font-bold tracking-tight">{leads.length}</span>
      </div>
      ...
      <div className="glass-panel p-6 flex flex-col gap-2 relative overflow-hidden group">
        ...
        <span className="text-sm font-medium text-slate-400">Available Properties</span>
        <span className="text-4xl font-bold tracking-tight">{properties.filter(p => p.status === 'Available').length}</span>
      </div>
      ...
      <div className="glass-panel p-6 flex flex-col gap-2 relative overflow-hidden group">
        ...
        <span className="text-sm font-medium text-slate-400">Revenue Projected</span>
        <span className="text-4xl font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-green-500">+ $1.2M</span>
      </div>
    </div>
  );
}
