    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-semibold text-n-100 flex items-center gap-2">
          ...
          Follow-up Tasks
          {pending.length > 0 && (
            <span className="badge-blue text-[10px]">{pending.length}</span>
          )}
        </h4>
      ...
      {showAdd && (
        <div className="space-y-2 p-3 bg-n-900/70 border border-n-500/60 rounded-lg">
          <input
          ...
          <div className="flex gap-2">
            ...
            />
            <button onClick={createTask} className="btn-primary text-xs px-3 py-1.5 shrink-0">Save</button>
          </div>
        </div>
      )}
      ...
      <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
        {tasks.length === 0 && !showAdd && (
          <p className="text-center text-n-500 text-sm py-6">No tasks pending.</p>
        )}
        {pending.map(t => (
          <div key={t.id} className="flex items-center gap-3 p-3 bg-n-800/50 rounded-lg border border-n-500/40 group hover:border-n-400 transition-colors">
            <button
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm text-n-100 truncate">{t.title}</p>
              {t.due_at && <p className="text-[10px] text-n-500 mt-0.5">Due {new Date(t.due_at).toLocaleDateString()}</p>}
            </div>
          </div>
        ))}
        {completed.length > 0 && (
          <details className="group/details">
            <summary className="text-xs text-n-500 cursor-pointer list-none flex items-center gap-1 py-1">
            ...
            <div className="space-y-1.5 mt-2">
              {completed.map(t => (
                <div key={t.id} className="flex items-center gap-3 p-2.5 rounded-lg border border-n-500/20 opacity-50">
                  <button onClick={() => toggleStatus(t)} className="w-4 h-4 rounded bg-accent-500/30 border border-accent-500/50 flex items-center justify-center shrink-0">
                  ...
                  <p className="text-xs text-n-500 line-through">{t.title}</p>
                </div>
              ))}
            </div>
          </details>
